...
public class BinaryTree<T> implements DefaultBinaryTree<T> {
    // public static java.util.List<Integer> list = new ArrayList<Integer>();
    ...
    public int findingMin(String bracketStr) throws Exception {
        fromBracketNotation(bracketStr);
        SimpleTreeNode tree = root;
        if (root == null) {
            return Integer.MIN_VALUE;
        }


        int res = Integer.parseInt(tree.value.toString());
        while (tree.left != null) {
            int lres = Integer.parseInt(tree.left.value.toString());
            if (lres < res) {
                res = lres;
            }
